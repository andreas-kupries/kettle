...
proc kt::check {name version} {
	...
	puts "- $name [package present $name]"
	return
    }
...
proc testsNeedTcl {version} {
    ...
    if {[package vsatisfies [package provide Tcl] $version]} {
...
proc mainPath {fname} {
    return [file join $::testutils::main $fname]
}
...
proc useMainFile {fname} {
    return [uplevel 1 [list source [mainPath $fname]]]
}
...
proc use {fname pname args} {
    ...
    if {[catch {
	uplevel 1 [list useMainFile $fname]
    } msg]} {
	puts "    Aborting the tests found in \"[file tail [info script]]\""
...
## Use some binary package ...
proc useC {glob name {outsideallowed yes}} {
    global dir

puts "|$glob $name $outsideallowed|"

    # The sorting usually puts the best match (with version
    # immediately after the fixed part), at the front. If not, we
...
puts Y|$index
    } msg] || ![file exists $index]} {
puts Z|$msg|
	if {$outsideallowed} {
	    # Might have an external package
	    if {![catch {
